...
var seedReviews = (amount) => {
  ...
  let seedIt = () =>{
    ...
    let paraNum = faker.random.number(5);
    let t = {
    ...
    let params = [t.description, t.title, t.languageID, t.locationID, t.userLocationID, t.userID, t.travelDate, t.travelType, t.ratingOverall, t.ratingExpenses, t.ratingLocation, t.ratingRooms, t.ratingService, t.ratingClean, t.ratingSleep, t.userThoughts, t.userTips];
    //TODO - add the photo urls
    db.connection.query('INSERT INTO Reviews (description, title, language_ID, location_ID, user_location_ID, user_ID, travel_date, travel_type, rating_overall, rating_expenses, rating_location, rating_rooms, rating_service, rating_clean, rating_sleep, user_thoughts, user_tips) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', params, (err, data) => {
      if (err) {
...
var seedLocationPhotos = () => {
  ...
  let photocount = faker.random.number(100);
  if (photocount > 90) {
    photocount = 3;
  ...
  let photos = [];
  while (photos.length < photo) {
    let id = faker.random.number(20);
    if (id = 0) id = 1;
    if (id < 10) id = '0' + id;
    let currentPhoto = `https://hrr43fecheskett.s3.us-east-2.amazonaws.com/${id}.jpg`
  }
}
